# bca_analyzer_lambda.py
# AWS Lambda version - accepts S3 path input, returns Excel as byte array
...
def download_from_s3(s3_path: str) -> bytes:
    ...
    print(f"Downloading from S3: bucket={bucket_name}, key={object_key}")
...
def create_excel_bytes(personal_df, summary_df, trx_df, partner_trx_df, analytics_df) -> bytes:
...
def lambda_handler(event, context):
    ...
    {
        "s3Path": "s3://bucket-name/path/to/statement.pdf"
    }
    ...
    Returns:
    {
        "statusCode": 200,
        "body": {
            "excelData": "<base64-encoded-excel-bytes>",
            "fileName": "BCA_Statement_Analysis_...",
            "metadata": {
                "accountName": "...",
                "accountNumber": "...",
    ...
    try:
        ...
        if not s3_path:
            return {
                'statusCode': 400,
                'body': json.dumps({
                    'error': 'Missing required parameter: s3Path'
                })
            }

        # Download PDF from S3
        pdf_bytes = download_from_s3(s3_path)
        ...
        excel_bytes = create_excel_bytes(personal_df, summary_df, trx_df, partner_trx_df, analytics_df)
        ...
        account_name = get_cell(personal_df, "Account Name", "Unknown")
        branch = get_cell(personal_df, "Branch", "Unknown")
        currency = get_cell(personal_df, "Currency", "IDR")

        base_name = f"BCA_Statement_Analysis_{period}_{account_no}".strip("_")
        output_filename = safe_filename(base_name) + ".xlsx"
        ...
        metadata = {
            ...
            'period': period,
            'currency': currency,
            'transactionCount': len(trx_df),
            'excelSizeBytes': len(excel_bytes)
        }

        if not analytics_df.empty:
            metadata['totalCreditAmount'] = float(get_cell(analytics_df, 'Total_Credit_Amount', 0))
            metadata['totalDebitAmount'] = float(get_cell(analytics_df, 'Total_Debit_Amount', 0))
            metadata['uniquePartners'] = int(get_cell(analytics_df, 'Total_Partners', 0))

        if not summary_df.empty:
            metadata['saldoAwal'] = float(get_cell(summary_df, 'Saldo Awal', 0)) if get_cell(summary_df, 'Saldo Awal', None) is not None else None
            metadata['saldoAkhir'] = float(get_cell(summary_df, 'Saldo Akhir', 0)) if get_cell(summary_df, 'Saldo Akhir', None) is not None else None
            metadata['mutasiKredit'] = float(get_cell(summary_df, 'Mutasi Kredit', 0)) if get_cell(summary_df, 'Mutasi Kredit', None) is not None else None
            metadata['mutasiDebet'] = float(get_cell(summary_df, 'Mutasi Debet', 0)) if get_cell(summary_df, 'Mutasi Debet', None) is not None else None

        print(f"Successfully processed. Metadata: {json.dumps(metadata)}")
...
if __name__ == "__main__":
    # Test event
    test_event = {
        's3Path': 's3://my-bucket/statements/test.pdf'
    }
